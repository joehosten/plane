...
function renderMentions(html: string): string {
  return html
    .replace(
      ...
      '<span class="inline-flex items-center rounded-md bg-amber-500/15 px-1.5 py-0.5 text-amber-700 font-medium dark:text-amber-300">$2</span>'
    )
    .replace(
      /<span[^>]*data-mention-type="user_mention"[^>]*>([^<]*)<\/span>/g,
      '<span class="inline-flex items-center rounded-md bg-amber-500/15 px-1.5 py-0.5 text-amber-700 font-medium dark:text-amber-300">$1</span>'
    );
}
